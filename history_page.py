...
st.caption("历史记录｜主账号查看全员，分账号仅查看本人｜Supabase 持久化")

if st.session_state["role"] == "主账号(Admin)":
    with st.expander("恢复 / 导入旧历史 CSV", expanded=False):
        st.caption("用于把以前下载的历史 CSV 恢复到永久历史库。重复记录不会重复新增。")
        old_history_file = st.file_uploader(
            "选择历史 CSV",
            type=["csv"],
            key="history_restore_csv",
        )
...
if history.empty:
    ...
    with h1:
        if st.session_state["role"] == "主账号(Admin)":
            operators = ["全部"] + sorted(
                [
                    value
                    for value in filtered["operator"].unique()
                    if value
                ]
            )
            ...
            if operator_filter != "全部":
                filtered = filtered[
                    filtered["operator"] == operator_filter
                ]
    ...
    with h2:
        record_types = ["全部"] + sorted(
            [
                value
                for value in filtered["record_type"].unique()
                if value
            ]
        )
        ...
        if type_filter != "全部":
            filtered = filtered[
                filtered["record_type"] == type_filter
            ]

    with h3:
        account_options = ["全部"] + sorted(
            [
                value
                for value in filtered["tiktok_account"].unique()
                if value
            ]
        )
        ...
        if account_filter != "全部":
            filtered = filtered[
                filtered["tiktok_account"] == account_filter
            ]
    ...
    st.dataframe(
        filtered[available_columns].iloc[::-1],
        hide_index=True,
        use_container_width=True,
    )
